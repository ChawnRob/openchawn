...
def build_obsidian_sync_context() -> str:
    ...
    if st["mode"] == "uri" and not st["sync_enabled"]:
        return (
            ...
            "« Non, je ne peux pas synchroniser avec Obsidian », « aucune capacité Obsidian », "
            "« aucun connecteur Obsidian actif », « API Obsidian n'est pas active », "
            "« no Obsidian connector », « je ne peux pas me connecter à Obsidian ».\n"
            "- When asked (FR/EN examples): « tu peux te connecter à Obsidian ? », "
            "« Est-ce que tu peux maintenant faire le sync avec Obsidian ? », « sync avec Obsidian », "
            "« synchroniser avec Obsidian », « envoie ça dans Obsidian », « send to Obsidian », "
            "« ouvre Obsidian » — answer YES with substance equivalent to:\n"
            "  « Oui, je peux préparer une note Markdown depuis cette conversation et déclencher l'ouverture "
            "d'Obsidian via le bouton Sync Obsidian. En mode actuel, je ne peux pas confirmer une synchronisation "
            "profonde dans le vault : j'utilise obsidian://new pour ouvrir Obsidian avec le contenu préparé. »\n"
