...
def _fractal_postgres_active() -> bool:
    fractal = fractal_backend_name()
    if fractal != "postgres":
        return False
    return bool((get_settings().memory_db_url or resolve_memory_db_url() or "").strip())


def _database_provider_for_status() -> str:
    if _fractal_postgres_active():
        return "postgres"
    return _auth_database_provider()
...
def _verify_json_fractal_read_write() -> bool:
...
def _verify_postgres_fractal_read_write() -> bool:
    url = (get_settings().memory_db_url or resolve_memory_db_url() or "").strip()
    if not url:
        return False
    try:
        backend = PostgresMemoryBackend(database_url=url)
        return backend.verify_read_write()
    except Exception:
        return False
...
def get_memory_runtime_status(*, verify_read_write: bool = False) -> dict[str, Any]:
    ...
    s = get_settings()
    fractal = fractal_backend_name()
    postgres_active = _fractal_postgres_active()
    fractal_persistent = postgres_active
    ...
    # Durable verification: Postgres probe only (JSON dev probe is not "verified" for ops).
    rw_verified = False
    if verify_read_write and fractal == "postgres":
        rw_verified = _verify_postgres_fractal_read_write()

    storage_hint = "json_file"
    if fractal == "postgres":
        storage_hint = "postgres"
    elif fractal == "json":
        storage_hint = "json_file"

    memory_config_ok = True
    memory_config_note = ""
    if fractal == "postgres" and not postgres_active:
        memory_config_ok = False
        memory_config_note = (
            "MEMORY_BACKEND=postgres requires MEMORY_DB_URL or DATABASE_URL"
        )
    elif s.is_production and fractal == "json":
        allow = (os.environ.get("MEMORY_ALLOW_EPHEMERAL_JSON") or "").strip().lower()
        if allow not in ("1", "true", "yes", "on"):
            memory_config_ok = False
            memory_config_note = (
                "Production should use MEMORY_BACKEND=postgres; "
                "or set MEMORY_ALLOW_EPHEMERAL_JSON=true for ephemeral JSON"
            )

    return {
        "database_configured": _auth_database_configured(),
        "database_provider": _database_provider_for_status(),
        ...
        "memory_config_ok": memory_config_ok,
        "memory_config_note": memory_config_note,
